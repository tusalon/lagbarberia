...
Si no puedes asistir, contactanos por WhatsApp al +53 ${telefonoDuenno}`;
        ...
        if (!confirm(`¿Estás seguro que querés cancelar tu turno del ${fechaConDiaConfirm} a las ${window.formatTo12Hour ? window.formatTo12Hour(bookingData.hora_inicio) : bookingData.hora_inicio}?`)) {
            return;
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-amber-50 to-amber-100 pb-20">
            ...
            <div className="bg-white/90 backdrop-blur-sm shadow-sm sticky top-0 z-10 border-b border-amber-200">
                <div className="max-w-3xl mx-auto px-4 py-4 flex justify-between items-center">
                    <button
                        onClick={onVolver}
                        className="flex items-center gap-2 text-amber-600 hover:text-amber-800 transition"
                    >
                        <i className="icon-arrow-left text-xl"></i>
                        <span className="font-medium">Volver</span>
                    </button>
                    <h1 className="text-xl font-bold text-amber-800">⚡ Mis Reservas ⚡</h1>
            ...
            <div className="max-w-3xl mx-auto px-4 py-6">
                ...
                <div className="bg-white/80 backdrop-blur-sm border border-amber-200 rounded-lg p-4 mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-medium text-amber-800">{cliente.nombre}</p>
                            <p className="text-sm text-amber-700">{cliente.whatsapp}</p>
                        </div>
                    </div>
                </div>
                ...
                {mensajeError && (
                    <div className="bg-amber-100 border border-amber-300 text-amber-700 p-3 rounded-lg mb-4 text-sm">
                        {mensajeError}
                    </div>
                )}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12 bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-amber-200">
                        ...
                        <p className="text-amber-700 mb-2">No tenés reservas {filtro !== 'todas' ? filtro : ''}</p>
                        <button
                            onClick={onVolver}
                                    ...
                                    <div className="p-4">
                                        <div className="flex justify-between items-start mb-3">
                                            <div>
                                                ...
                                                <h3 className="font-bold text-amber-800 text-lg">{booking.servicio}</h3>
                                            </div>
                                            <span className={`
                                                px-3 py-1 rounded-full text-xs font-semibold
                                                ${booking.estado === 'Reservado' ? 'bg-amber-100 text-amber-700' :
                                                  booking.estado === 'Confirmado' ? 'bg-amber-200 text-amber-800' :
